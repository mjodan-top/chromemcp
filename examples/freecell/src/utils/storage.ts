...
// 记录游戏开始
export function recordGameStart(playerData: PlayerData): PlayerData {
  const newData = {
    ...playerData,
...
// 记录游戏失败
export function recordGameLoss(playerData: PlayerData): PlayerData {
  const newData = {
    ...playerData,
    currentStreak: 0
  };
...
// 计算星级 (1-3星)
function calculateStars(time: number, moves: number): number {
  // 时间评分 (秒)
...
function isBetterScore(
  time: number,
  moves: number,
  current: { time: number; moves: number }
): boolean {
  // 优先比较移动次数，其次时间
  if (moves < current.moves) return true;
  if (moves === current.moves && time < current.time) return true;
  return false;
}