import { SUIT_COLORS, SUIT_SYMBOLS, RANK_DISPLAY } from '../types';
// 创建一副牌 (52张)
...
// 使用种子洗牌
export function shuffleWithSeed(deck, seed) {
    const shuffled = [...deck];
...
// 判断两张牌颜色是否不同
export function isOppositeColor(card1, card2) {
    return getSuitColor(card1.suit) !== getSuitColor(card2.suit);
}
// 判断是否可以叠放在游戏区 (tableau)
// 规则: 颜色交替，点数递减
export function canStackOnTableau(movingCard, targetCard) {
...
// 判断是否可以放入空位 (freeCell)
export function canMoveToFreeCell(freeCells) {
    return freeCells.some(cell => cell === null);
}
// 计算可移动的最大卡牌数
// 公式: (空位数 + 1) * (空列数 + 1)
export function getMaxMovableCards(freeCells, tableau, excludeColumn) {
    const emptyFreeCells = freeCells.filter(cell => cell === null).length;
    const emptyColumns = tableau.filter((col, idx) => col.length === 0 && idx !== excludeColumn).length;
