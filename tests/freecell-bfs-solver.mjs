#!/usr/bin/env node
/**
 * FreeCell Level 1 - Optimized BFS Solver
 * 使用优化的BFS搜索，限制搜索深度和状态数量
 */
...
// 简化的状态哈希 - 只关注Foundation和顶部牌
function stateHash(state) {
  const { tableau, freeCells, foundations } = state;
  const fd = foundations.map(f => f.length).join(',');
  const tops = tableau.map(col => col.length > 0 ? `${col[col.length-1].rank}${col[col.length-1].suit[0]}` : '_').join('');
...
function priority(state) {
  ...
  return foundationTotal * 100 + canMoveNow * 10;
}
...
function getMoves(state) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (card && canMoveToFoundation(card, foundations)) {
      moves.push({ type: 'fc_to_f', card: {...card}, from: i, to: getSuitIndex(card.suit), priority: 0 });
    }
  }
  ...
  for (let col = 0; col < 8; col++) {
    ...
    if (canMoveToFoundation(topCard, foundations)) {
      moves.push({ type: 't_to_f', card: {...topCard}, from: col, to: getSuitIndex(topCard.suit), priority: 1 });
    }
  }
    ...
    for (let targetCol = 0; targetCol < 8; targetCol++) {
      ...
      if (targetColumn.length > 0) {
        ...
        if (canStackOnTableau(topCard, targetCard)) {
          ...
          moves.push({ type: 't_to_t', card: {...topCard}, from: col, to: targetCol, priority: willExpose ? 2 : 5 });
        }
      }
    }
  }
    ...
    for (let targetCol = 0; targetCol < 8; targetCol++) {
      ...
      if (targetColumn.length > 0) {
        ...
        if (canStackOnTableau(card, targetCard)) {
          moves.push({ type: 'fc_to_t', card: {...card}, from: i, to: targetCol, priority: 4 });
        }
      }
    }
  }
  ...
  for (let col = 0; col < 8; col++) {
    ...
    if (topCard.rank === 13 && emptyCols.length > 0) {
      const willExpose = column.length > 1 && canMoveToFoundation(column[column.length - 2], foundations);
      moves.push({ type: 't_to_t', card: {...topCard}, from: col, to: emptyCols[0], priority: willExpose ? 3 : 8 });
    }
  }

  // Tableau -> FreeCell (只暴露有用牌)
  if (emptyFC !== -1) {
    for (let col = 0; col < 8; col++) {
      const column = tableau[col];
      if (column.length <= 1) continue;
      const topCard = column[column.length - 1];
      const willExpose = column.length > 1 && canMoveToFoundation(column[column.length - 2], foundations);
      if (willExpose || column[column.length - 2]?.rank === 1) {
        moves.push({ type: 't_to_fc', card: {...topCard}, from: col, to: emptyFC, priority: 3 });
      }
    }
  }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (card && card.rank === 13 && emptyCols.length > 0) {
      moves.push({ type: 'fc_to_t', card: {...card}, from: i, to: emptyCols[0], priority: 9 });
    }
  }
...
// 优化的BFS搜索
function bfsSolve(initialState, maxStates = 500000, maxDepth = 120) {
  console.log('Starting optimized BFS search...');

  // 使用优先队列
  const queue = [];
  const visited = new Map(); // hash -> min depth

  // 初始状态
  const initialHash = stateHash(initialState);
  queue.push({ state: initialState, path: [], depth: 0 });
  visited.set(initialHash, 0);
  ...
  while (queue.length > 0 && statesExplored < maxStates) {
    ...
    // 取出优先级最高的状态
    queue.sort((a, b) => priority(b.state) - priority(a.state));
    const current = queue.shift();

    // 检查胜利
    if (isWin(current.state)) {
      console.log(`Found solution! Path length: ${current.path.length}, States explored: ${statesExplored}`);
      return current.path;
    }
    ...
    if (foundationTotal > bestFoundation) {
      ...
      console.log(`  Progress: ${bestFoundation}/52, Depth: ${current.depth}, Queue: ${queue.length}`);
    }
    ...
    for (const move of moves) {
      ...
      // 检查是否已访问或更优路径
      if (!visited.has(hash) || visited.get(hash) > newDepth) {
        visited.set(hash, newDepth);
        queue.push({
          state: nextState,
  ...
  console.log(`\nNo solution found. Best: ${bestFoundation}/52, States explored: ${statesExplored}`);
  return null;
}
...
async function main() {
  console.log('🃏 FreeCell Level 1 - Optimized BFS Solver\n');
  ...
  try {
    ...
    // 运行BFS搜索
    const solution = bfsSolve(initialState, 1000000, 150);
