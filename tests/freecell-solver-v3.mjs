...
function getBestMove(state, previousMoves) {
  ...
  // 3. 从Tableau移动到Tableau (建立序列，有价值的优先)
  for (let col = 0; col < 8; col++) {
    const column = tableau[col];
    if (column.length === 0) continue;
    const topCard = column[column.length - 1];
  ...
  for (let col = 0; col < 8; col++) {
    ...
    for (let targetCol = 0; targetCol < 8; targetCol++) {
      ...
      if (tableau[targetCol].length === 0) {
        const priority = topCard.rank === 13 ? 40 : 20;
        moves.push({
          type: 'tableau_to_tableau',
          card: topCard,
          fromCol: col,
  ...
  if (emptyFreeCellIndex !== -1) {
    for (let col = 0; col < 8; col++) {
      ...
      moves.push({
        ...
        priority: willExposeA ? 80 : (willExposeUseful ? 65 : 15)
      });
    }
  }
  ...
  if (moves.length > 0) {
    for (const move of moves) {
      ...
      const recentCount = previousMoves.slice(-8).filter(m => m === moveKey).length;
      if (recentCount < 3) {
        return move;
      }
    }
...
async function main() {
  console.log('🃏 FreeCell Level 1 - Complete Solver (Internal API v3)\n');
