#!/usr/bin/env node
/**
 * FreeCell Level 1 - Improved DFS Solver
 * 改进的DFS搜索，增加迭代限制和更好的启发式
 */
...
// DFS with memoization - 改进版
function dfsSolve(initialState, maxDepth = 180) {
  ...
  if (!result) {
    console.log(`\nNo complete solution found. Best: ${bestFoundation}/52`);
    console.log(`Returning best path with ${bestPath.length} moves...`);
    return bestPath;
  }
...
async function main() {
  console.log('🃏 FreeCell Level 1 - Improved DFS Solver\n');
  ...
  try {
    ...
    // 运行DFS搜索
    const solution = dfsSolve(initialState, 180);
    ...
    if (foundationTotal === 52) {
      ...
      console.log(`\n已移动 ${foundationTotal} 张牌到Foundation`);
