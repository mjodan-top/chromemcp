...
function getAllValidMoves(state) {
    ...
    for (let targetCol = 0; targetCol < 8; targetCol++) {
      ...
      if (targetColumn.length > 0) {
        ...
        if (canStackOnTableau(topCard, targetCard)) {
          ...
          let priority = 50;
          if (willExposeA) priority = 90;
          else if (willExposeFoundation) priority = 80;
          moves.push({ type: 'tableau_to_tableau', card: topCard, fromCol: col, toCol: targetCol, priority });
        }
      }
    }
  }
    ...
    for (let targetCol = 0; targetCol < 8; targetCol++) {
      ...
      if (targetColumn.length > 0) {
        ...
        if (canStackOnTableau(card, targetCard)) {
          moves.push({ type: 'freecell_to_tableau', card, freeCellIndex: i, toCol: targetCol, priority: 55 });
        }
      }
    }
  }
  ...
  if (emptyFreeCellIndex !== -1) {
    for (let col = 0; col < 8; col++) {
      ...
      if (willExposeFoundation || willExposeA) {
        moves.push({ type: 'tableau_to_freecell', card: topCard, fromCol: col, freeCellIndex: emptyFreeCellIndex, priority: willExposeA ? 85 : 75 });
      }
    }
  }
  ...
  for (let col = 0; col < 8; col++) {
    ...
    const topCard = column[column.length - 1];
    if (topCard.rank !== 13 && emptyColumns.length > 0) {
      moves.push({ type: 'tableau_to_tableau', card: topCard, fromCol: col, toCol: emptyColumns[0], priority: 20 });
    }
  }

  // 9. 任意牌移到FreeCell (最低优先级)
  if (emptyFreeCellIndex !== -1) {
    for (let col = 0; col < 8; col++) {
      const column = tableau[col];
      if (column.length <= 1) continue;
      const topCard = column[column.length - 1];
      moves.push({ type: 'tableau_to_freecell', card: topCard, fromCol: col, freeCellIndex: emptyFreeCellIndex, priority: 10 });
    }
  }
  ...
  try {
    ...
    while (moveCount < maxMoves) {
      ...
      if (foundationTotal > lastFoundationTotal) {
        console.log(`\n📈 Foundation进展: ${foundationTotal}/52 [${state.foundations?.map(f => f.length).join('/')}]`);
      ...
      for (const move of moves) {
        ...
        const recentCount = moveHistory.slice(-20).filter(m => m === moveKey).length;
        if (recentCount < 3) {
          selectedMove = move;
          break;
        }
      ...
      moveCount++;
      const moveKey = `${selectedMove.type}-${selectedMove.card.rank}-${selectedMove.card.suit}`;
      moveHistory.push(moveKey);
      if (moveHistory.length > 50) moveHistory.shift();

      console.log(`[${moveCount}] ${selectedMove.card.rank}${selectedMove.card.suit[0].toUpperCase()} - ${selectedMove.type} (P:${selectedMove.priority})`);
    ...
    console.log(`Foundation: ${foundationTotal}/52 [${state.foundations?.map(f => f.length).join('/')}]`);
